...
func parseData(d *schema.ResourceData) (*Grant, error) {
	...
	allowedResouceTypesForPriviledge := privilegeToResourceTypesMap[privilege]
	...
	if !match_found {
		return nil, fmt.Errorf("%s resource not applicable for privilege %s - valid resource_types are %s", resource_type, privilege, strings.Join(allowedResouceTypesForPriviledge, ", "))
	}
...
func resourceGrantRead(d *schema.ResourceData, meta interface{}) error {
	...
	d.Set(identifier_privilege, grant.Priviledge)
